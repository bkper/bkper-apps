...
export function dataTableToCsv(rows: CsvCell[][], delimiter: CsvDelimiter): string {
    return rows.map(row => row.map(cell => serializeCell(cell, delimiter)).join(delimiter)).join('\r\n');
}
...
function serializeCell(cell: CsvCell, delimiter: CsvDelimiter): string {
    ...
    const mustQuote = text.includes(delimiter) || text.includes('"') || text.includes('\r') || text.includes('\n');
