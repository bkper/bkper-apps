...
describe('bot service', () => {
    ...
    it('limits concurrent pending-task requests to five and preserves Book order', async () => {
        const books = Array.from({ length: 6 }, (_, index) =>
            createBook(`book-${index + 1}`, { exc_code: `C${index + 1}` })
        );
        const pendingBookIds = new Set(['book-2', 'book-4', 'book-6']);
        const resolvers = new Map<string, (backlog: Backlog) => void>();
        let markLastRequestStarted = (): void => {};
        const lastRequestStarted = new Promise<void>(resolve => {
            markLastRequestStarted = resolve;
        });
        ...
        for (const book of books) {
            book.getBacklog = mock(
                () =>
                    new Promise<Backlog>(resolve => {
        ...
        const pendingBooksPromise = botService.getBooksWithPendingTasks(new Set(books));
        ...
        for (const book of books.slice(0, 5).reverse()) {
            ...
            resolve(new Backlog({ count: pendingBookIds.has(book.getId()) ? 1 : 0 }));
        }
        ...
        lastResolve(new Backlog({ count: 1 }));

        expect(Array.from(await pendingBooksPromise, book => book.getId())).toEqual([
            'book-2',
            'book-4',
            'book-6',
        ]);
