...
export class BotService {
        ...
        if (collectionBooks) {
            for (const collectionBook of collectionBooks) {
                if (
                    ...
                    this.getBaseCode(collectionBook) != 'TEMPLATE'
                ) {
                    books.push(collectionBook);
                }
            }
        }
    ...
    async getAccountExcCode(book: Book, account: bkper.Account): Promise<string | undefined> {
        const groups = account.groups;
        if (groups) {
            for (const group of groups) {
                const excCode = await this.getGroupExcCode(book, group);
                if (excCode) {
                    return excCode;
                }
            }
    ...
    private async getGroupExcCode(book: Book, group: bkper.Group): Promise<string | undefined> {
        const exchangeBooks = await this.getConnectedBooks(book);
        for (const exchangeBook of exchangeBooks) {
            const bookExcCode = this.getBaseCode(exchangeBook);
    ...
    async extractAmountDescription_(
        baseBook: Book,
        connectedBook: Book,
        base: string,
        connectedCode: string,
        transaction: bkper.Transaction,
        ratesEndpointUrl: string
    ): Promise<AmountDescription> {
        if (ratesEndpointUrl == null) {
            throw 'exchangeRatesUrl must be provided.';
        }
        const rates = await this.exchangeService.getRates(ratesEndpointUrl);
        const amountDescription = await this.getAmountDescription_(
            baseBook,
            connectedBook,
            base,
            connectedCode,
    ...
    private async getAmountDescription_(
        baseBook: Book,
        connectedBook: Book,
        base: string,
        connectedCode: string,
        ...
        if (
            txExcAmount &&
            (connectedCode == txExcCode || this.match(baseBook, connectedCode, transaction))
        ) {
        ...
        if (
            txExcRate &&
            (connectedCode == txExcCode ||
                this.isBaseBook(connectedBook) ||
                this.match(baseBook, connectedCode, transaction))
        ) {
        ...
        const convertedAmount = await this.exchangeService.convert(
