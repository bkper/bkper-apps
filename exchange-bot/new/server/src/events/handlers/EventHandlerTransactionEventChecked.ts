...
import { EXC_LOG_PROP, EXC_RATE_PROP } from '../../constants.js';
...
export class EventHandlerTransactionChecked extends EventHandlerTransactionEvent {
    ...
    protected async connectedTransactionFound(
        ...
        let amountDescription = await super.extractAmountDescription_(
            baseBook,
            connectedBook,
            baseCode!,
            connectedCode!,
        ...
        if (amountDescription.rates) {
            excLogEntries = await this.buildExcLog(
                baseBook,
                connectedBook,
                transaction,
                amountDescription
        ...
        if (
            this.amountHasChanged(connectedTransaction, amountDescription) ||
            excLogEntries.length > 0
        ) {
            if (this.amountHasChanged(connectedTransaction, amountDescription)) {
                connectedTransaction.setAmount(amountDescription.amount);
                connectedTransaction.setProperty(
                    EXC_RATE_PROP,
                    amountDescription.excBaseRate!.toString()
                );
            }
            ...
            !connectedTransaction.isPosted() &&
            this.isReadyToPost(connectedTransaction.json())
        ) {
    ...
    private amountHasChanged(
        ...
        return !currentAmount
            .round(NUM_OF_DECIMAL_PLACES)
            .eq(amountDescription.amount.round(NUM_OF_DECIMAL_PLACES));
    }
}
