...
export class EventHandlerTransactionUpdated extends EventHandlerTransactionEvent {
    ...
    protected async connectedTransactionNotFound(
        baseBook: Book,
        connectedBook: Book,
        baseTransaction: bkper.Transaction
    ): Promise<string | null> {
        ...
        let newTransaction = await super.mirrorTransaction(
            baseBook,
            connectedBook,
            baseTransaction
        );
    ...
    protected async connectedTransactionFound(
        ...
        await this.updateConnectedTransaction(
            baseBook,
            connectedBook,
            connectedTransaction,
            amountDescription,
            baseTransaction,
            connectedCreditAccount,
            connectedDebitAccount
        );
    ...
    private async updateConnectedTransaction(
        baseBook: Book,
        connectedBook: Book,
        connectedTransaction: Transaction,
        amountDescription: AmountDescription,
        transaction: bkper.Transaction,
        connectedCreditAccount: Account | undefined,
        connectedDebitAccount: Account | undefined
    ): Promise<void> {
        if (connectedTransaction.isChecked()) {
            await connectedTransaction.uncheck();
        }
        ...
        if (amountDescription.rates) {
            const excLogEntries = await this.buildExcLog(
                baseBook,
                connectedBook,
                transaction,
                amountDescription
