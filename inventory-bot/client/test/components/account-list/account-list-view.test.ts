...
describe('Account list view', () => {
    ...
    it('renders waiting, successful, and failed operation results after their Accounts', () => {
        ...
        expect(waiting.strings.join('')).toContain('aria-label="Processing ');
    ...
    it('renders the whole-Book eligible scope and an explicit empty state', () => {
        const renderedText = collectRenderedText(render.call(new AccountListView()));

        expect(renderedText).toContain('Eligible inventory accounts:');
