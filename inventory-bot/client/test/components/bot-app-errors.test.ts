...
describe('Bot app errors', () => {
    ...
    it('identifies Inventory Book access and permission failures', () => {
        const accessError = BotAppErrors.bookAccessRequired('inventory/id', 'the Inventory Book');
    ...
    it('builds the Inventory Bot installation action for the selected Book', () => {
        ...
        expect(error.message.action).toEqual({
            label: 'install',
            url: 'https://bkper.app/automations/book%2Fid/apps/inventory-bot',
        });
    });
