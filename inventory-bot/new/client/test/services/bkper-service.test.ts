...
describe('Bkper service', () => {
    it('loads the global App and enriches missing metadata from local constants', async () => {
        const app = new App({
            id: APP_ID,
            name: 'Global Inventory Bot',
            logoUrl: 'https://example.com/global-light.svg',
            website: 'https://example.com/inventory-bot',
        });
        ...
        expect(loadedApp.getName()).toBe('Global Inventory Bot');
        ...
        expect(loadedApp.getWebsiteUrl()).toBe('https://example.com/inventory-bot');
    ...
    it('loads an installed App by its universal id', async () => {
        ...
        const app = new App({ id: 'inventory-bot' });
        book.getApps = mock(async () => [app]);

        const loadedApp = await bkperService.loadInstalledApp(book, 'inventory-bot');
