import { Account, AccountType, Amount, type Book, type Transaction } from 'bkper-js';
...
export class BotService {
    ...
    getQuantity(transaction: bkper.Transaction): Amount | undefined {
        const quantityStr = transaction.properties?.[QUANTITY_PROP];
    ...
    async getFinancialBook(book: Book, excCode?: string): Promise<Book | undefined> {
        ...
        const connectedBooks = book.getCollection()!.getBooks();
        for (const connectedBook of connectedBooks) {
            const excCodeConnectedBook = this.getBookExcCode(connectedBook);
    ...
    async getExchangeCodeFromAccount(
        account: Account | bkper.Account
    ): Promise<string | undefined> {
        if (account instanceof Account) {
            if (
                account.getType() == AccountType.INCOMING ||
                account.getType() == AccountType.OUTGOING
            ) {
                return undefined;
            }
            ...
            if (groups != null) {
                for (const group of groups) {
                    const excCode = group.getProperty(EXC_CODE_PROP);
            ...
            if (groups != undefined) {
                for (const group of groups) {
                    const goodExchange = group.properties?.[EXC_CODE_PROP];
