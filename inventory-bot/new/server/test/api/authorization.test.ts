...
describe('API Book authorization', () => {
    ...
    test('requires Inventory Bot to be installed in the Book', async () => {
        const installedBook = new Book({ id: 'installed-book' });
        installedBook.getApps = async () => [new App({ id: 'inventory-bot' })];
        ...
        try {
            ...
            if (error instanceof HTTPException) {
                ...
                expect(error.message).toContain('Inventory Bot');
            }
        }
    });
});
