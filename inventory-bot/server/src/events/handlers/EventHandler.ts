...
export abstract class EventHandler {
    ...
    protected async intercept(_eventBook: Book, _event: bkper.Event): Promise<EventResult> {
        return { result: false };
    }
    ...
    async handleEvent(event: bkper.Event): Promise<EventResult> {
        ...
        const interceptionResponse = await this.intercept(eventBook, event);
        ...
        const inventoryBook = this.botService.getInventoryBook(eventBook);
