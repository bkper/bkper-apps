...
export class EventHandlerTransactionChecked extends EventHandlerTransaction {
    ...
    protected override async intercept(eventBook: Book, event: bkper.Event): Promise<EventResult> {
        return await new InterceptorFlagRebuild(this.context).intercept(eventBook, event);
    }
    ...
    protected async connectedTransactionNotFound(
        ...
        if (
            financialTransaction.debitAccount &&
            financialTransaction.creditAccount &&
            financialTransaction.date &&
            financialTransaction.id &&
            financialTransaction.properties
        ) {
            ...
            if (saleInvoice) {
                ...
                let inventoryAccount = await optionalLookup(() =>
                    inventoryBook.getAccount(goodProperty)
                );
                if (!inventoryAccount) {
                    inventoryAccount = await this.createConnectedInventoryAccountOnSale(
                        inventoryBook,
                        goodProperty
                    );
                }

                let inventorySellAccount = await optionalLookup(() =>
                    inventoryBook.getAccount(GOOD_SELL_ACCOUNT_NAME)
                );
                if (!inventorySellAccount) {
                    inventorySellAccount = await new Account(inventoryBook)
                        .setName(GOOD_SELL_ACCOUNT_NAME)
                ...
                const newTransaction = await new Transaction(inventoryBook)
                    ...
                    .setProperty(
                        SALE_INVOICE_PROP,
                        financialTransaction.properties[SALE_INVOICE_PROP]
                    )
                    .setProperty(ORDER_PROP, financialTransaction.properties[ORDER_PROP])
                    .setProperty(
                        SALE_AMOUNT_PROP,
                        new Amount(financialTransaction.amount ?? 0).toString()
                    )
                ...
                const needsRebuild = await this.checkLastTxDate(
                    inventoryAccount,
                ...
                let inventoryAccount = await optionalLookup(() =>
                    inventoryBook.getAccount(financialDebitAccount.name)
                );
                // if account is not found, create it
                if (!inventoryAccount) {
                    inventoryAccount = await this.createConnectedInventoryAccountOnPurchase(
                        inventoryBook,
                ...
                let inventoryBuyAccount = await optionalLookup(() =>
                    inventoryBook.getAccount(GOOD_BUY_ACCOUNT_NAME)
                );
                if (inventoryBuyAccount == null) {
                    inventoryBuyAccount = await new Account(inventoryBook)
                        .setName(GOOD_BUY_ACCOUNT_NAME)
                ...
                const newTransaction = await new Transaction(inventoryBook)
                    ...
                    .setProperty(
                        PURCHASE_CODE_PROP,
                        financialTransaction.properties[PURCHASE_CODE_PROP]
                    )
                ...
                let inventoryAccount = await optionalLookup(() =>
                    inventoryBook.getAccount(financialCreditAccount.name)
                );
                if (!inventoryAccount) {
                    inventoryAccount = await this.createConnectedInventoryAccountOnPurchase(
                        inventoryBook,
                ...
                const newTransaction = await new Transaction(inventoryBook)
                    ...
                    .setProperty(
                        CREDIT_NOTE_PROP,
                        financialTransaction.properties[CREDIT_NOTE_PROP]
                    )
                    .setProperty(
                        PURCHASE_CODE_PROP,
                        financialTransaction.properties[PURCHASE_CODE_PROP]
    ...
    private async checkLastTxDate(
        ...
        const lastTxDate = this.botService.getCOGSCalculationDateValue(goodAccount);
        if (
            lastTxDate != null &&
            transaction.dateValue != undefined &&
            transaction.dateValue <= +lastTxDate
        ) {
    ...
    private async createConnectedInventoryAccountOnPurchase(
        inventoryBook: Book,
        ...
        if (inventoryAccountGroups.length > 0) {
            for (const group of inventoryAccountGroups) {
                let inventoryGroup = await optionalLookup(() => inventoryBook.getGroup(group.name));
                if (!inventoryGroup) {
                        ...
                        .setParent(
                            group.parent?.name
                                ? await optionalLookup(() =>
                                      inventoryBook.getGroup(group.parent?.name)
                                  )
                                : null
                        )
    ...
    private async createConnectedInventoryAccountOnSale(
        inventoryBook: Book,
    ...
    private async checkInventoryAccount(
        inventoryBook: Book,
        financialAccount: bkper.Account
    ): Promise<boolean> {
        const inventoryAccount = await optionalLookup(() =>
            inventoryBook.getAccount(financialAccount.name)
        );
