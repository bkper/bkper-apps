...
- Apply the action-time pending-task guard and per-Account progress and outcome presentation to Forward Date.
- Reuse the shared confirmation dialog for the confirmed Forward Date batch before execution.
...
- Add deferred Account request batching only after its execution behavior is explicitly accepted; current operation execution remains sequential.
