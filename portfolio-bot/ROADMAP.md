...
**Chunks 1–23 complete — Chunk 24 repository consolidation is in progress; developer access restoration remains pending.**

The production baseline is recorded, the event-routing drift has been explicitly resolved in favor of the current `EventHandlerGroupDeleted` behavior, the unchanged legacy projects were isolated under `legacy/` during migration, and the full-stack Cloudflare skeleton, deterministic event dispatcher, shared event orchestration, common resolution boundaries, posted order processing, checked quantity mirroring, transaction lifecycle behavior, resource synchronization, and typed menu API contract were established in the Cloudflare target before consolidation. The Chunk 8 event matrix is complete with no unexplained event-side difference; the target SDK's Account–Group resolution, retry policy, and structured error behavior remain accepted. The target API exposes one read-only pending-calculation Account query and four Account-level operation routes with shared `200 OK` `{ message: string }` success responses. Chunk 10 has ported the legacy pending-calculation Account query, Book, Account, and Group context, authoritative Portfolio Book default date, structured Portfolio Book failures, originating and Portfolio Book view and installation checks, stale-state reset, Financial Book edit availability, and Full Reset view availability. Chunk 11 protects the pending-calculation API, resolves every mutation stub's Portfolio, Financial, and Base Book context, preflights edit permission and Portfolio Bot installation across that context, and enforces the Full Reset owner and unlocked-Collection boundary. Chunk 12 has completed regular and Full Reset parity, the shared operation-response contract, and both Reset operation routes with preflight and lock-failure translation. Chunk 13 has established the Calculate support types, `StockAccount` behavior, required `BotService` boundaries, ordered mutation processor, existing Calculate helper behavior, complete `processSale` parity, Calculate entry orchestration, and the wired Calculate API facade with the shared operation response and locked no-write translation. Reset and Full Reset now precede Calculate in Chunks 12 and 13 because the legacy Calculate rebuild branch invokes regular Reset and returns. Chunk 14 has established the Forward service structure, ported its remaining constants, `StockAccount` behavior, and `BotService` dependencies, completed regular Forward Date parity, ported lower-forward-date repair with its separate immediate and sequential Reset behavior, completed top-level Forward Date validation and branch parity, and wired the Forward API facade and route through the shared operation response. Chunk 15 is complete: the target client exposes Realized Results and Forward Date controls, and Calculate, regular Reset, Full Reset, and Forward Date are wired end to end through dedicated controllers with the action-time pending-task guard and per-Account progress and outcomes. Calculate preserves the current date and MTM intent, and Forward Date preserves the current date. Calculate, regular Reset, and Full Reset use bounded sliding concurrency of five Account requests, preserving ordered outcomes and continuing after individual Account failures without mutation retries; Forward Date remains sequential. Full Reset and Forward Date require explicit confirmation before execution. Non-mutating browser acceptance passed in managed Chromium across narrow, configured sidebar, and expanded widths; light and dark themes; standalone and embedded rendering; Account, Group, and pending-calculation scopes; confirmations; waiting, success, and error states; keyboard focus; and WCAG A and AA checks. Chunk 16 completed the frozen-install full-stack audit, repeated the complete deterministic matrices, reconciled every event and menu-operation boundary with the legacy sources, added focused coverage for fallback linked-Transaction deletion and aggregate versus exchange-specific Calculate support Accounts, confirmed the API, authorization, asynchronous, performance, dependency, workflow, and UI boundaries, and reproduced the client and Worker artifacts byte-for-byte across a clean rebuild. No unexplained accounting or event drift, new production patch, dependency exposure, or zero-sum risk was found. The embedded client now consumes Bkper's trusted `bkper:app-url-changed` message through explicit parent-source, Bkper-origin, message-shape, and target-App-origin validation. Trusted changes outside an Account operation update browser history and reload context without an iframe refresh; overlapping reloads retain only the newest context. Changes received while an Account operation is executing are ignored completely without changing history or UI, queuing the context, or replaying it after completion. Live preview acceptance from a real Bkper Book passed in Chunk 19. Dependency advisories were triaged as unreachable tooling-only findings, so no override or dependency churn was introduced. Chunk 17 built and deployed the frozen candidate to preview, narrowed developer access to the migration operator, routed the development menu and events independently, established the isolated multi-Book validation Collection, and verified authentication, assets, OpenAPI, API protection, authenticated read-only context, event ingress, and logs while production remained on GCP and GAS. Live menu acceptance exposed unresolved optional Account and Group URL expressions when no resource was selected; the client now treats those Bkper template expressions as absent, with focused deterministic coverage. The revised preview renders the accepted empty Portfolio Book context, and the complete local gate passes with 138 client tests and 187 server tests. Chunk 18 restored the preview event route after detecting stale local-tunnel drift, then completed live event validation across isolated Instruments, EUR Financial, and USD Base Books. Purchase and sale splitting, fees, interest, quantity mirroring, duplicate prevention, transaction lifecycle behavior, order cascade deletion and restoration, Account, Group, and Book synchronization, missing and unsupported no-ops, zero-quantity failure, and loop prevention all produced the accepted active resources and relationships. No incomplete active movement, duplicate active remote id, reversed persisted movement, partial cleanup, or nonzero per-Book total remained. Production routing remains unchanged. Chunk 20 repeated the authoritative legacy and target drift audits, reconciled the patch ledger without a new production patch, passed the clean frozen local gate with 149 client tests and 187 server tests, reproduced the five deployment artifacts byte-for-byte, and deployed the accepted Worker to production. The production OpenAPI contract, API protection, authenticated client boundary, asset manifest, and logs are available. Chunk 21 changed only the production webhook to Cloudflare, completed its one-hour active monitoring window without a warning, error, non-success request, or rollback indicator, and completed the 24-hour read-only stabilization window while the production menu remained on GAS. The first 24 hours contained 8,089 production event requests: every request completed with a Platform `ok` outcome and HTTP `200`, while five bounded application-level errors comprised one upstream idle timeout and four Account-name conflicts. The errors showed no sustained growth, 1,832 additional post-barrier requests were all `info`, `ok`, and HTTP `200`, and no customer impact was reported. Stabilization was accepted without accessing or writing customer Book data. Chunk 23 moved the production menu to the already-deployed Cloudflare client and completed its 24-hour full-stack stabilization window. The window contained 27,529 requests: all 569 web/API requests were `info`, Platform `ok`, and HTTP `200`, including 560 Account mutations; 26,958 of 26,960 event requests were Platform `ok` and HTTP `200`. Five inherited missing-Portfolio-Book deletion failures, one upstream idle timeout, and two isolated canceled event requests were retained as bounded operational signals without sustained growth. A further 1,364 post-barrier requests were all `info`, Platform `ok`, and HTTP `200`. Stabilization was accepted without using logs as proof of accounting outcomes or accessing customer Books for monitoring.

Cloudflare is production-authoritative for events and the Portfolio Bot menu, and the active application occupies the Portfolio Bot project root. The previous GCP and Apps Script source remains recoverable from Git history. The unchanged Google Apps Script web app remains available for immediate menu rollback, and the unchanged Google Cloud Function remains available for immediate event rollback.
...
During migration, target metadata kept production `menuUrl` and `webhookUrl` on Apps Script and GCP while development URLs moved independently to Cloudflare preview. The target preserved the unpublished App's production access policy with `users: '*@brain.uy'`, which remains in the consolidated root `bkper.yaml`.
...
The previous GCP and GAS source remains recoverable from Git history. Their unchanged deployments remain independent routing rollback targets until separately retired. References below to `legacy/` and `new/` describe the historical pre-consolidation source locations.
...
| Transaction restore commentary | Core represents a trashed Transaction with its Account endpoints reversed while it is non-balance-affecting, and both the deployed and target SDKs refresh only the `trashed` flag on `untrash()` | Preserve the inherited response behavior during migration and verify the authoritative reloaded Transaction rather than inferring restored direction from stale commentary | Any response-presentation hardening remains separate post-migration work |
...
This chunk is a parity port of the legacy batched `resetRealizedResultsForAccountAsync` behavior used by the Reset and Full Reset menu operations and by Calculate's `needs_rebuild` branch. The target method is named `ResetRealizedResultsService.execute(context, full)`; the class already supplies the realized-result context, the shared operation context carries its Portfolio, Financial, and Base Book resources, and target methods are asynchronous. The separate immediate and sequential legacy `resetRealizedResultsForAccountSync` behavior is used only by lower-forward-date repair; its target method is named `executeSync`, it remains with the Forward Date port in Chunk 14, and it must not be deduplicated with the batched implementation during migration.
...
- Extended the existing `BotService` only with methods directly required by Calculate.
- Preserved calculation-model selection, next-date and FIFO ordering, historical, fair, forwarded, explicit, stored, and replicated price and rate precedence, gain calculations, Base Book checks, and complete replicated-rate pagination.
- Ported interest and support Account lookup, creation, type inference, and common-Group inference with awaited SDK boundaries and optional Account 404 handling; the Calculate API remains non-mutating and unwired.
- Verified the complete local gate with 95 client tests, 140 server tests, strict typechecks, production client and Worker builds, formatting, and generated-file drift checks.

Subchunk 3 evidence:

- Ported `CalculateRealizedResultsProcessor` with its four legacy Maps, two MTM Sets, remote-id keys, id-based replacement, temporary UUID ids, and lock detection.
- Preserved regular and historical MTM date and movement-direction accumulation with the required asynchronous Account-name adaptation.
...
- Ported the complete `processSale` method as one parity unit in `CalculateRealizedResultsService`, composing the support class without introducing another calculation layer or changing helper behavior.
- Preserved the legacy variable names, comments, branch order, property writes, relationships, and mutation order across multiple lots, complete and partial matches, long and short sales, Portfolio splits, all three calculation models, and MTM behavior.
- Limited target adaptations to the class boundary, `Amount` and `Transaction` construction, awaited SDK and composed-support calls, and strict nullable SDK assertions. A token comparison against the legacy method is exact after reversing those adaptations.
- Added eleven deterministic tests covering complete and partial long and short matches, multiple lots, checked-lot skipping, purchase and sale splits, all calculation models, helper ordering, date and price fallbacks, absent exchange rates, fraction-digit residuals, and composed realized and FX movements; every asserted split or result movement is complete and retains its accepted direction.
...
- Wired Calculate through the thin `calculate-service.ts` facade after the established context resolution, authorization, installation, and complete Portfolio, Financial, and Base Book loading boundaries.
- Passed the requested date and MTM intent to the ported account-level Calculate orchestration and returned its commentary through the shared `200 OK` operation response without adding mutation tracking or resource receipts.
- Translated the inherited locked no-write Summary outcome to the structured `400` API error while leaving other Summary outcomes as successful operation messages.
- Added focused facade and route coverage and verified the complete local gate with 95 client tests, 169 server tests, strict typechecks, production client and Worker builds, formatting, and generated-file drift checks.
...
This chunk is a strict parity port of the complete legacy `ForwardDateService` behavior and of the separate immediate and sequential legacy `resetRealizedResultsForAccountSync` behavior, named `executeSync` in the target, used only by lower-forward-date repair. Every validation, branch, query, recursive lookup, balance read, property, relationship, remote id, state transition, no-op, delay, mutation, failure boundary, and return outcome remains aligned with legacy unless an unavoidable target-runtime adaptation is explicitly recorded. The target awaits asynchronous SDK work but does not batch, parallelize, reorder, deduplicate, or redesign the established Forward sequence.
...
- Established `forward/forward-date-service.ts` with an unwired, non-mutating operation stub and left the existing API facade unchanged.
- Added the two remaining Forward constants and ported only the Forward-required `StockAccount` identity, forwarded-state, and sequential trash-cleanup behavior.
- Extended the existing `BotService` with Book-backed date formatting, `Buy` and `Sell` Account lookup and creation, and complete paginated Account-level uncalculated-result validation. Target adaptations are limited to awaited SDK boundaries, explicit nullable Account handling, and passing the already-resolved Book and Account instead of reloading them by id.
- Verified the complete local gate with 95 client tests and 173 server tests, strict typechecks, production client and Worker builds, formatting, and generated-file drift checks.

Subchunk 2 evidence:

- Ported regular Forward Date as one ordered parity unit in `ForwardDateService`, retaining the established balance reads, FIFO input selection, immediate history-log post and source update sequence, liquidation bridge, batch check, unrealized-balance reads, forwarded result, Portfolio Account update, five-second pre-closing delay, and optional Portfolio Book closing-date update.
- Preserved complete purchase, sale, liquidation, and forwarded-result movements with their established directions, amounts, properties, relationships, descriptions, and remote ids. Incomplete source logs remain incomplete and non-balance-affecting.
- Limited target adaptations to resolved operation context, complete cursor pagination, awaited SDK work, nullable SDK values, deterministic ISO closing-date calculation, and an interceptable delay. The Forward API facade remains non-mutating and unwired.
- Verified the complete local gate with 95 client tests and 179 server tests, strict typechecks, production client and Worker builds, formatting, and generated-file drift checks.

Subchunk 3 evidence:

- Ported the separate legacy `resetRealizedResultsForAccountSync` behavior as target `executeSync` without deduplicating it with the batched Reset implementation, retaining immediate source-order uncheck, linked cleanup, trash, update, Account-state, and completion behavior across Portfolio, Financial, and Base Books.
- Ported lower-forward-date repair with the established current-forward Reset, forwarded-Transaction query, recursive previous-state lookup, source restoration, superseded-log cleanup, requested-date Reset, re-forwarding, and final message order.
- Preserved owner and open and unlocked Collection checks before the first mutation, complete cursor pagination, optional previous-state lookup, and sequential awaited SDK operations without batching, parallelization, response tracking, route wiring, or API facade integration.
- Verified the complete local gate with 95 client tests and 182 server tests, strict typechecks, production client and Worker builds, formatting, and generated-file drift checks.

Subchunk 4 evidence:

- Ported `ForwardDateService.execute` with the legacy uncalculated-result, equal-forwarded-date, lower-forward-date, realized-date, and regular-forward branch order and messages.
- Restored the owner and open and unlocked Collection checks to their authoritative top-level lower-date branch before repair begins, while retaining the completed lower-date and regular Forward implementations unchanged.
- Preserved the inherited rebuild validation inside regular Forward and kept the non-mutating API facade, route, schemas, generated contract, and response tracking unchanged and unwired.
- Verified the complete local gate with 95 client tests and 184 server tests, strict typechecks, production client and Worker builds, formatting, and generated-file drift checks.

Subchunk 5 evidence:

- Wired `ForwardService.execute` to the completed `ForwardDateService` after the established context resolution, authorization, installation, and Financial chart loading boundaries.
- Passed the requested date to Forward, returned its commentary through the shared `200 OK` operation response, and translated typed Forward validation outcomes to the structured `400` API error without adding mutation tracking or resource receipts.
- Replaced the route's empty placeholder message with the facade response while leaving schemas, OpenAPI, and generated client types unchanged.
- Verified the complete local gate with 95 client tests and 185 server tests, strict typechecks, production client and Worker builds, formatting, and generated-file drift checks.
...
- Wired regular Reset through the Realized Results controller and authenticated generated API client with the same action-time pending-task guard, sequential Account order, no mutation retries, continuation after individual failures, and per-Account outcomes.
- Wired Full Reset through the Forward Date controller and authenticated generated API client, rendered it only when available and to the left of Forward as a danger action, and retained its owner and open and unlocked Collection availability boundary.
- Consolidated the common pending-task guard, waiting-state initialization, sequential Account execution, startup-error handling, control restoration, and per-Account request outcome behavior without changing operation-specific validation or API intent.
- Added a shared Web Awesome confirmation dialog with an optional typed-phrase guard, wired Full Reset through the `FULL RESET` danger confirmation, and retained the existing operation lifecycle only after explicit confirmation.
- Wired Forward Date end to end through a dedicated Lit reactive controller and the authenticated generated API client, preserving the current date, sequential Account requests, execution-disabled controls, and top-level operation failures.
- Applied the action-time Portfolio Book pending-task guard to Forward Date before its first Account request and added waiting, completed, and error outcomes per Account while retaining sequential order and continuation after individual failures.
...
- Reinstalled the target with Bun 1.3.0 and the committed frozen lockfile. The install resolved 367 packages without changing the lockfile; workspace scripts used the pinned local Bkper CLI 4.24.3 rather than the separately installed global CLI.
- Repeated the complete generated-contract, strict typecheck, client, server, production-build, formatting, and generated-file drift gate. All 137 client tests and 187 server tests pass without credentials, external requests, or live Book access.
- Reconciled all thirteen event routes, the event class and method inventory, common resolution, order processing, quantity mirroring, lifecycle cleanup, and resource synchronization with the current legacy GCF source. The only event differences remain the already accepted Platform authentication, structured SDK errors, modern SDK lookup and retry behavior, current-source Group deletion baseline, and awaited Worker-lifecycle completion.
- Reconciled the active GAS Calculate, Reset, Full Reset, and Forward Date boundaries with the target operation services, processors, API facades, and client controllers. FIFO, complete and partial lots, short sales, support Accounts, realized, historical, FX, MTM, interest-MTM, reset, forward, lower-date repair, canonical relationships, mutation phases, and per-Account outcomes retain deterministic coverage.
- Added one focused event test for ordered uncheck, rebuild flagging, and trash of a connected fallback movement, and one focused Calculate test for aggregate and exchange-specific regular and historical support Account selection. Production behavior and contracts did not change.
- Confirmed that every mutating API route validates typed input, resolves and authorizes its complete operation context, verifies Portfolio Bot installation, and completes preflight before its first write. Server code creates request-scoped `Bkper` instances without inbound token or API-key providers, and the client does not retry mutation requests after authentication or operation failures.
- Confirmed the accepted bounded Account concurrency of five for Calculate and both Reset operations, sequential Forward Date execution, one action-time pending-task check, ordered outcomes, and continuation after isolated Account failures. Live trusted parent-context changes remain explicitly deferred to Chunk 19 and are not treated as accepted production readiness.
- Re-ran dependency triage. The frozen graph still reports the same eight advisories across `undici`, `nanoid`, `esbuild`, and `js-yaml`; path and bundle inspection confirmed the affected versions and operations remain local tooling-only, while the separate Bkper CLI path resolves the safe `undici` 8.9.0. No dependency, override, or lockfile change was justified.
- Built the client and Worker again after removing `dist/`; all five generated files were byte-for-byte identical to the preceding build. The candidate contains a 443.53 kB client JavaScript asset, a 55.43 kB stylesheet, and a 1.37 MB Worker bundle before gzip where applicable, with no unexpected advisory package bundled into deployed runtime behavior.
- Reconciled the migration patch ledger with no new production-authoritative patch or unexplained source drift. Updated target guidance to reflect Chunk 16 completion. No sync, deployment, routing, installation, event replay, credential use, or Book mutation occurred.
...
Completed preview-readiness evidence:

- Rebuilt from a clean Bun 1.3.0 frozen install, passed the complete deterministic gate, and deployed the reviewed candidate to preview without deploying or rerouting production.
- Temporarily restricted Portfolio Bot developer access to the migration operator, routed the development menu first, verified authenticated read-only Portfolio context, and only then routed development events to preview.
- Established one isolated Collection with an Instruments Book, a EUR Financial Book, and a USD Base/Financial Book. Portfolio Bot is installed on all three Books; the already-accepted Exchange Bot preview is installed only on the two Financial Books.
- Confirmed the generated OpenAPI contract, unauthenticated API and event protection, authenticated read-only pending-calculation query, static client rendering, preview request logs, and one controlled no-movement event delivery.
- Live embedded acceptance exposed that Bkper preserves `${account.id}` and `${group.id}` literally when no Account or Group is selected. Added focused URL-boundary coverage, treated unresolved Bkper menu expressions as absent, repeated the complete gate with 138 client tests and 187 server tests, redeployed preview, and accepted the corrected empty Instruments scope.
- Verified through the CLI that all three synthetic Books remained at zero Transactions after readiness validation. No Calculate, Reset, Full Reset, Forward Date, order, quantity, or monetary movement was initiated.
- Kept production menu and webhook routing on GAS and GCP. Preview developer access remains intentionally narrowed for controlled event validation.
...
Completed preview-event evidence:

- Re-established the reviewed preview event route after authoritative metadata inspection found that a later local development session had replaced it with an inactive tunnel. Replayed only the already-created no-movement Group event after the route was corrected; production menu and webhook routing remained on GAS and GCP.
- Started from an isolated three-Book Collection with zero Transactions, then exercised eligible and ineligible Group and Account creation, Account rename and archive, transaction-free Account deletion, Group rename, hidden state and property replacement, transaction-free Group deletion, and historical Book-property propagation and cleanup. Resource synchronization created no movement.
- Posted complete purchase and sale orders with fees and interest. The resulting Financial movements retained the accepted directions and exact decomposition, and the installed Exchange Bot produced the independently balanced Base-Book equivalents without Portfolio Bot reprocessing Exchange Bot events.
- Checked purchase and sale instrument movements and obtained exactly one linked `Buy >> instrument` and `instrument >> Sell` quantity movement. Replaying a checked event returned the existing mirror without creating a duplicate.
- Exercised Financial uncheck, description-only update, instrument deletion and restoration, complete original-order cascade deletion, original-order restoration, and checking the rebuilt instrument movement. Active and trashed generations retained canonical remote ids and exactly one active relationship; unrelated movements remained untouched.
- Exercised missing quantity, recognized zero quantity, unsupported order shape, Portfolio Bot agent-loop prevention, and Exchange Bot loop prevention. Missing and unsupported inputs remained complete source movements without derived Portfolio movements; zero quantity produced the accepted error before any derived movement.
- One source trash request timed out before mutation. Authoritative Transaction, Event, derived-resource, and preview-log reads confirmed that no write or delivery occurred before a later fresh request completed; no uncertain retry was performed.
- Confirmed through authoritative resource reloads that trashed Transactions are non-balance-affecting and restored Transactions return to their accepted persisted directions. The inherited SDK restore commentary can retain the trashed payload's reversed Account order, so that response-only quirk is recorded separately and was not mistaken for movement drift.
- The final deterministic audit found two active Instruments movements, eleven active EUR movements, and eleven active USD Base movements; every active movement was complete, no active remote id was duplicated, and each Book summed exactly to zero. The only preview error in the controlled window was the intentional zero-quantity case.
- Repeated the complete local gate with 138 client tests and 187 server tests, strict typechecks, production client and Worker builds, formatting, and generated-file drift checks.

**Gate:** No duplicate, missing, reversed, partial, or imbalanced posted movement was found.
...
- Rebuilt and deployed the clean committed candidate to preview without changing production metadata. The five-path OpenAPI contract, API protection, authenticated root behavior, preview menu and event routes, and unchanged production GAS and GCP routes remained accepted.
- Live embedded Book, Account, Group, and query changes followed the parent context without leaving a stale actionable resource. Context, permission presentation, validation, and final client interactions were accepted in the installed preview.
- On isolated synthetic Books, FIFO partial-long, complete-long, and short-cover behavior passed under combined, fair-only, and historical-only models. Fair-only and historical-only acceptance used canonical Financial orders and Portfolio Bot-authored quantity movements. Source splits, parent relationships, checked state, gain properties, model-specific remote-id prefixes, realized results, historical results, FX results, MTM, and interest-MTM retained their accepted amounts and directions.
- Combined calculation passed both without and with MTM. Fair-only emitted only standard fair-value realized and FX relationships. Historical-only emitted historical values through the standard historical-only relationships without duplicate fair or historical-prefix results.
- Regular Reset restored each canonical source and removed its split and linked results under fair-only and historical-only models. Full Reset, regular Forward Date, lower-date repair, forward-state cleanup, and MTM cleanup restored their accepted lifecycle and Account state.
- A locked Calculate and a closed regular Reset each returned the accepted structured `400` before any Transaction or Account write. A live downstream-Book `VIEWER` check returned `403` before accounting behavior and changed no Transaction or Account; the temporary permission and ownership arrangement was fully restored.
- A missing-rate purchase remained unchecked and unresolved after Calculate returned its accepted asynchronous success commentary. It gained no exchange-rate property, created no result movement, and was removed after Full Reset restored Account state.
- Every accepted scenario was verified from authoritative Book state rather than HTTP success alone. Active movements were complete, remote ids were unique, FIFO relationships and directions were canonical, and each Book summed exactly to zero. Final cleanup restored the documented active baseline, source amounts and properties, open dates, model flags, permissions, ownership, and preview and production routes.
- The complete local gate passes with 149 client tests and 187 server tests, strict typechecks, production client and Worker builds, formatting, and generated-file drift checks.

Rollout concerns:

- One Full Reset returned `200` while Exchange Bot timed out deleting one propagated Base-Book `#stock_loss_fwd` movement. Replaying only that failed Exchange Bot response completed cleanup and the final state was correct. During rollout, operation success must not be treated as propagation proof: monitor dependent bot responses and authoritative Base-Book state, and replay only a confirmed failed response when necessary.
- Preview automation temporarily returned `Plan upgrade required!` while creating a canonical fixture. The failed generation was removed without replay; after preview plan availability was restored, a fresh generation completed normally. Confirm automation plan availability before each cutover or canary.
- Direct CLI-authored Portfolio movements are outside Reset's established Portfolio Bot-agent ownership boundary. Live Reset acceptance therefore used canonical Financial orders whose derived Portfolio movements were authored by Portfolio Bot; the temporary direct fixtures were removed and did not establish product drift.

**Gate:** Trusted preview context transitions cannot leave a stale actionable scope, and the target workflows and accounting outcomes are accepted with documented API, UI, asynchronous propagation, and rollout differences.
...
- Repeated the authoritative GCF source and deployed-artifact audit against the active Gen 2 deployment. Its runtime, entry point, resource limits, and immutable source remain unchanged from the migration baseline. Forty-eight of fifty generated JavaScript and source-map artifacts match the current frozen legacy rebuild byte-for-byte; the only differences remain `index.js` and its source map, caused solely by the already-accepted `GROUP_DELETED` routing correction from the older deployed artifact to `EventHandlerGroupDeleted`.
- Confirmed that the production GAS web app remains on application version 1.16.0 and project version 174. Remote HEAD and the deployed version contain the same sixteen files, and all sixteen locally generated artifacts match them byte-for-byte under the accepted TypeScript 4.9.5 and Bkper Apps Script library version 201 toolchain.
- Confirmed that no legacy GCF or GAS implementation, package, or lockfile changed after the migration baseline. Reconciled the patch ledger: the accepted Group-deletion dispatch and target SDK Account–Group behavior remain implemented and covered, with no new production patch or unexplained drift.
- Removed the target dependencies and build output, installed from the frozen lockfile, and passed generated contracts, strict client and server typechecks, 149 client tests, 187 server tests, production client and Worker builds, formatting, and generated-file drift checks.
- Rebuilt the client and Worker a second time and reproduced all five deployment artifacts byte-for-byte. The dependency audit also retained exactly the eight previously triaged tooling-only advisories, with no new deployed exposure or dependency change.
- Confirmed that persisted App metadata matches the target configuration before deployment. Production webhook and menu routes remained on GCP and GAS, development routes remained on preview, and the accepted Worker was deployed to production without syncing metadata or writing to a Book.
- Confirmed production runtime availability, the five-path OpenAPI contract, unauthenticated API rejection, authenticated client and asset protection, and successful production request logging. The working tree remained clean after deployment.

**Gate:** Deployment changed runtime availability only; production event and menu authority remain on GCP and GAS.
...
- Changed only the production webhook route from GCP to Cloudflare. Persisted routing was verified while the production menu remained on GAS, both development routes remained on preview, and the unchanged GCP function remained available for immediate event rollback.
- Completed the one-hour active-monitoring window. Exact bounded log aggregation found 1,183 production `POST /events` requests; every request was classified `info`, `ok`, and HTTP `200`, with no warning, error, non-success request, or rollback indicator. Traffic was bursty and included activity throughout the window.
- Completed the agreed 24-hour read-only observation period. Exact bounded aggregation found 8,089 production `POST /events` requests; all had a Platform `ok` outcome and HTTP `200`, 8,084 were classified `info`, and five contained application-level errors.
- Classified the five errors without retaining customer or resource identifiers: one upstream 30-second idle timeout and four Account-name conflicts occurred in two bounded clusters. They produced no sustained authentication, runtime, or error growth. HTTP `200` remained transport evidence rather than proof that those handler executions completed their business behavior.
- Observed 1,832 additional production event requests after the 24-hour barrier; every request was classified `info`, `ok`, and HTTP `200`, with no warning or application-level error.
- Received no customer-impact report during the stabilization window. No customer Book was accessed, inspected, or written for monitoring or error investigation, and no Worker deployment or production routing change occurred during observation.
- Kept accounting correctness grounded in the retained deterministic matrix and accepted preview evidence; production logs were not used to infer movements, balances, or zero-sum correctness.
- Accepted event stabilization with Cloudflare production-authoritative for events, GAS still production-authoritative for the menu, and GCP retained as the event rollback target.
...
### Chunk 22 — Client UX revision and production redeploy

**Status: Complete.**

Three deliberate target client UX improvements were committed after the Chunk 20 production deployment and before the menu cutover. They are recorded here so the menu-cutover audit finds no unexplained client drift.

- The Full Reset confirmation replaces the typed-phrase input with an acknowledgement checkbox (`I understand and want to proceed`). The danger action remains disabled until the checkbox is checked, the acknowledgement clears between openings, and the legacy GAS no-confirmation baseline remains exceeded. The plain Forward confirmation is unchanged.
- The service-switcher no longer renders instructions. Each feature view owns its intro copy in a two-paragraph `.intro` block with bold action names, following the Exchange Bot pattern.
- Realized Results and Forward Date intro copy is action-oriented and grounded in the authoritative services: Calculate matches unchecked sales against their purchases FIFO and records the resulting realized and exchange results; Reset undoes already calculated results; Forward moves open trades to the chosen date at the current price; Full Reset removes all results and forward states.

Drift audit against the Chunk 20 deployed baseline:

- No server, script, package, or lockfile change exists since the Chunk 20 production deployment; the only production-affecting differences are the three client commits, the already-stabilized Chunk 21 webhook routing change in `bkper.yaml`, and a removed unused `.dev.vars.example` placeholder.
- The production `menuUrl` remains on GAS.
- The complete local gate passes with the revised candidate: 149 client tests and 187 server tests, strict client and server typechecks, production client and Worker builds, OpenAPI generation, formatting, and no generated-file drift.

The revised candidate was built and deployed to preview and then to production with byte-identical artifacts and no routing change: the production `menuUrl` remains on GAS and production events remain on the already-stabilized Cloudflare Worker. The acknowledgement checkbox and intro copy changes were reviewed by the migration operator as part of committing them, satisfying live acceptance for this revision. No Book was read or written for this chunk.

### Chunk 23 — Production menu cutover and full-stack stabilization

**Status: Complete.**

Completed evidence:

- Passed the complete deterministic gate immediately before cutover with 149 client tests and 187 server tests, strict client and server typechecks, production client and Worker builds, formatting, and no generated-file drift. The reviewed candidate changed only the production `menuUrl` from GAS to the already-deployed Cloudflare client.
- Synced only App metadata on 2026-09-03. Production deployment artifacts were not changed, no Book write was initiated for cutover or monitoring, both development routes remained on preview, production events remained on Cloudflare, and the unchanged GAS and GCP deployments remained available as independent rollback targets. The active-monitoring epoch began at `2026-09-03T17:03:30Z`.
- Verified persisted routing, unchanged production and preview deployments, production OpenAPI availability, unauthenticated client redirection, protected API rejection, and authenticated production context loading. Three production pending-calculation context requests completed with `info`, Platform `ok`, and HTTP `200`; no production Account mutation request had occurred by the early review.
- Completed the exact first-hour monitor in twelve five-minute windows. Four web/API requests were all `info`, Platform `ok`, and HTTP `200`. Of 271 production event requests, all had Platform `ok` outcomes and HTTP `200`; 270 were `info`, and one carried an application-level error from the inherited missing-Portfolio-Book transaction-deletion path.
- Traced that first error to the retained deletion interceptor ordering and nullable Portfolio Book lookup shared by the legacy GCF and Cloudflare target. It was not introduced by the menu cutover, did not recur during the early window, and an event rollback would retain the same behavior. Worker request identifiers were redacted, and the matching Book was not available through the migration operator's read-only event access. The issue remains deferred rather than mixed into migration stabilization.
- Through `2026-09-03T18:56:00Z`, reviewed 1,500 additional production event requests in bounded windows. All retained Platform `ok` and HTTP `200`; 1,499 were `info`, while one isolated upstream 30-second idle timeout matched the already-accepted event-stabilization error class. Neither observed error showed sustained growth.
- The migration operator's developer menu continued to route to preview as configured. A live batch of 17 Calculate and 17 Forward Date Account requests completed there with `info`, Platform `ok`, HTTP `200`, and no API error. This remains supporting evidence from the byte-identical preview candidate rather than production menu-mutation evidence.
- Completed the exact 24-hour observation from `2026-09-03T17:03:30Z` through `2026-09-04T17:03:30Z` with 27,529 production requests. All 569 web/API requests were `info`, Platform `ok`, and HTTP `200`: eight pending-calculation reads, 386 Calculate requests, 173 Forward Date requests, one Reset request, and one OpenAPI read. No Full Reset or non-successful API response appeared.
- Of 26,960 production event requests, 26,952 were `info`; 26,958 had Platform `ok` outcomes and HTTP `200`. The eight error-level records comprised five inherited missing-Portfolio-Book deletion failures, one previously accepted upstream 30-second idle timeout, and two isolated canceled requests with no detail or HTTP status. The inherited failures stopped after a bounded cluster, the cancellations were separated by more than fifteen hours, and none of these signals showed sustained growth. Canceled requests were not treated as proof of handler completion.
- Three otherwise successful requests exposed transient upstream behavior and recovered through the target SDK policy: one Forward Date request and one event request retried an upstream `500`, and one event request retried an upstream connection termination. Each completed with `info`, Platform `ok`, and HTTP `200`; operation success was not treated as authoritative proof of dependent Book state.
- Observed 1,364 additional production requests after the 24-hour barrier through `2026-09-04T18:32:00Z`: 1,330 events and 34 web/API requests comprising six pending-calculation reads, 27 Calculate requests, and one Reset request. Every request was `info`, Platform `ok`, and HTTP `200`, with no warning, error, cancellation, retry signal, or non-success response.
- Reverified the persisted production menu and event routes, preview routes, and unchanged production and preview deployment timestamps after the barrier. Monitoring performed no customer Book read or write. Accounting correctness and the zero-sum invariant remain grounded in the retained deterministic matrix and authoritative preview evidence, not production request logs.
- Explicitly accepted full-stack stabilization. Cloudflare remains production-authoritative for the menu and events, GAS remains the immediate menu rollback target, and GCP remains the immediate event rollback target.
...
### Chunk 24 — Repository consolidation and deferred legacy retirement

**Status: Repository structure consolidated; developer access restoration pending.**

- Moved the accepted Cloudflare client, Worker, tests, configuration, lockfile, generated contracts, scripts, and project instructions from `new/` to the Portfolio Bot root.
- Removed the inactive `legacy/` working-tree source and obsolete local GCP and GAS tooling. The previous source remains recoverable from Git history.
- Updated workspace instructions to classify Portfolio Bot as a production Platform app, retained Vite `5179` and Worker `8797`, removed the retired local GCF port `3002`, and aligned workspace port forwarding.
- Passed the complete Cloudflare gate before and after consolidation with 149 client tests and 187 server tests, strict typechecks, production client and Worker builds, formatting, and generated-contract checks.
- Compared all 169 unchanged active target files with their pre-move Git contents and found no mismatch. The five production deployment artifacts also remained byte-for-byte identical across the move.
- Kept the unchanged deployed GCP and Apps Script runtimes available as independent routing rollback targets. Performed no App sync, deployment, routing change, Book operation, event replay, legacy infrastructure mutation, or remote deletion during consolidation.
- Normal team-wide developer access remains temporarily restricted to the migration operator. Updating and syncing that access policy requires a separate explicit approval before Chunk 24 is complete.

**Gate:** The repository-structure gate passed: Cloudflare is the only active implementation in the project root, and consolidation changed no application behavior or remote state. Chunk 24 remains open only for normal developer-access restoration.
