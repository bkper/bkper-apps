...
- Kept accounting correctness grounded in the retained deterministic matrix and accepted preview evidence; HTTP success alone is not movement proof.

Stabilization underway:

- Continue the agreed 24-hour read-only observation period with periodic production event-log reviews and customer-impact monitoring.
- Do not initiate customer Book writes solely for monitoring, deploy a new Worker, or change either production route during the observation window.
- Keep the production menu on GAS and GCP available for event rollback until stabilization is explicitly accepted.
